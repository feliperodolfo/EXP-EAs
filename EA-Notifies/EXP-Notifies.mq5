...
input NOTIFY hab_ma_notify = SIM;
...
input NOTIFY hab_mom_vshape = SIM;
...
input NOTIFY hab_obv_vshape = SIM;
...
input int rsi_ma_period = 9;     // Período da MA do RSI
input ENUM_MA_METHOD rsi_ma_method = MODE_SMA; // Método da MA do RSI
input NOTIFY hab_rsi_vshape = SIM;   // Habilita V-Shape
input NOTIFY hab_rsi_notify = SIM;
...
input int stoch_ma_period = 9;   // Período da MA do Stoch
input ENUM_MA_METHOD stoch_ma_method = MODE_SMA; // Método da MA do Stoch
input NOTIFY hab_stoch_vshape = SIM; // Habilita V-Shape
input NOTIFY hab_stoch_notify = SIM; 

sinput string s5; //----------Volume Parameters-------------
input ENUM_TIMEFRAMES volume_time_frame = PERIOD_H2;  // TimeFrame do Volume
input int volume_ma_period = 9;
input ENUM_MA_METHOD volume_ma_method = MODE_EMA;
input NOTIFY hab_volume_notify = SIM;
...
int ma_fast_handle; double ma_fast_buffer[];
int ma_med_handle;  double ma_med_buffer[];
int ma_slow_handle; double ma_slow_buffer[];
...
bool IsRevertingUp(double &m[]) {
   if(ArraySize(m) < 12) return false;
