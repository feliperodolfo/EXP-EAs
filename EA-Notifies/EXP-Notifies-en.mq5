...
#property description "- OBV forms a reversal pattern (V-Shape);"
#property description "- RSI exits overbought or oversold limits or forms a V-Shape pattern;"
#property description "- Stochastic exits overbought or oversold limits or forms a V-Shape pattern;"
#property strict
...
void OnTick()
{
   ...
   string msgFinal = Symbol() + ", " + EnumToString(Period()) + " at: " + TimeToString(TimeLocal()) + ". by EXP-Notifies";
   ...
   if (isNewBarVol)
   {
      if (ena_volume_notify == YES)
      {
             ...
             CopyBuffer(volume_ma_handle, 0, 0, 3, volumeMABuffer) > 0)
         {
            ...
            {
               SendNotification("Volume Aggression (" + EnumToString(volume_time_frame) + ") - " + msgFinal);
               Alert("Volume Aggression (" + EnumToString(volume_time_frame) + ") - " + msgFinal);
            }
         }
      }
   }
   ...
   if (isNewBarMain)
   {
      ...
      if (ena_ma_notify == YES)
      {
         ...
         if (CopyBuffer(ma_fast_handle, 0, 0, 2, ma_fast_buffer) > 0)
         {
            ...
            {
               SendNotification("Price touched MA " + IntegerToString(ma_fast_period) + " - " + msgFinal);
               Alert("Price touched MA " + IntegerToString(ma_fast_period) + " - " + msgFinal);
            }
         }
         if (CopyBuffer(ma_med_handle, 0, 0, 2, ma_med_buffer) > 0)
         {
            ...
            {
               SendNotification("Price touched MA " + IntegerToString(ma_med_period) + " - " + msgFinal);
               Alert("Price touched MA " + IntegerToString(ma_med_period) + " - " + msgFinal);
            }
         }
         if (CopyBuffer(ma_slow_handle, 0, 0, 2, ma_slow_buffer) > 0)
         {
            ...
            {
               SendNotification("Price touched MA " + IntegerToString(ma_slow_period) + " - " + msgFinal);
               Alert("Price touched MA " + IntegerToString(ma_slow_period) + " - " + msgFinal);
            }
         }
      }
          ...
          CopyBuffer(stoch_ma_Handle, 0, 0, 15, stoch_ma_Buffer) > 0)
      {
         ...
         if (ena_stoch_notify == YES)
         {
            ...
            {
               SendNotification("Stochastic exited overbought region - " + msgFinal);
               Alert("Stochastic exited overbought region - " + msgFinal);
            }
            if (stoch_Buffer[1] > stoch_oversold && stoch_Buffer[2] < stoch_oversold)
            {
               SendNotification("Stochastic exited oversold region - " + msgFinal);
               Alert("Stochastic exited oversold region - " + msgFinal);
            }
         }
         // V-Shape Stoch
         if (ena_stoch_vshape == YES)
         {
            ...
            {
               SendNotification("Stochastic V-Shape - Bullish reversal - " + msgFinal);
               Alert("Stochastic V-Shape - Bullish reversal - " + msgFinal);
            }
            if (IsRevertingDown(stoch_ma_Buffer))
            {
               SendNotification("Stochastic V-Shape - Bearish reversal - " + msgFinal);
               Alert("Stochastic V-Shape - Bearish reversal - " + msgFinal);
            }
         }
      }
          ...
          CopyBuffer(rsi_ma_Handle, 0, 0, 15, rsi_ma_Buffer) > 0)
      {
         ...
         if (ena_rsi_notify == YES)
         {
            ...
            {
               SendNotification("RSI exited overbought region - " + msgFinal);
               Alert("RSI exited overbought region - " + msgFinal);
            }
            if (rsi_Buffer[1] > rsi_oversold && rsi_Buffer[2] < rsi_oversold)
            {
               SendNotification("RSI exited oversold region - " + msgFinal);
               Alert("RSI exited oversold region - " + msgFinal);
            }
         }
         // V-Shape RSI
         if (ena_rsi_vshape == YES)
         {
            ...
            {
               SendNotification("RSI V-Shape - Bullish reversal - " + msgFinal);
               Alert("RSI V-Shape - Bullish reversal - " + msgFinal);
            }
            if (IsRevertingDown(rsi_ma_Buffer))
            {
               SendNotification("RSI V-Shape - Bearish reversal - " + msgFinal);
               Alert("RSI V-Shape - Bearish reversal - " + msgFinal);
            }
         }
      }
      ...
      if (ena_mom_vshape == YES)
      {
         if (CopyBuffer(mom_ma_handle, 0, 0, 15, mom_ma_Buffer) > 0)
         {
            ...
            {
               SendNotification("Momentum V-Shape - Bullish reversal - " + msgFinal);
               Alert("Momentum V-Shape - Bullish reversal - " + msgFinal);
            }
            if (IsRevertingDown(mom_ma_Buffer))
            {
               SendNotification("Momentum V-Shape - Bearish reversal - " + msgFinal);
               Alert("Momentum V-Shape - Bearish reversal - " + msgFinal);
            }
         }
      }
      ...
      if (ena_obv_vshape == YES)
      {
         if (CopyBuffer(obv_ma_handle, 0, 0, 15, obv_ma_Buffer) > 0)
         {
            ...
            {
               SendNotification("OBV V-Shape - Bullish reversal - " + msgFinal);
               Alert("OBV V-Shape - Bullish reversal - " + msgFinal);
            }
            if (IsRevertingDown(obv_ma_Buffer))
            {
               SendNotification("OBV V-Shape - Bearish reversal - " + msgFinal);
               Alert("OBV V-Shape - Bearish reversal - " + msgFinal);
            }
         }
      }
   }
